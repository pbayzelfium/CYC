# Generates one zelfium-<slug>.omp.json per palette in palettes.json, by
# remapping the colour tokens used in zelfium.omp.json.
# Re-run after editing zelfium.omp.json or palettes.json.

# its own folder, not $HOME: the script lives beside the files it reads, so it
# works wherever the setup is installed
$root     = $PSScriptRoot
$base     = Get-Content (Join-Path $root 'zelfium.omp.json') -Raw
$palettes = Get-Content (Join-Path $root 'palettes.json') -Raw | ConvertFrom-Json

# the literal hexes as written in zelfium.omp.json (the catppuccin-mocha values)
$src = [ordered]@{
...
foreach ($p in $palettes.PSObject.Properties) {
    ...
    $path = Join-Path $root "zelfium-$slug.omp.json"
